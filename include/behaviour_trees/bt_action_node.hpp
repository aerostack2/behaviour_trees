    ...
    BT::NodeStatus tick() override
    {
        ...
        try
        {
            ...
            if (rclcpp::ok() && !goal_result_available_)
            {
                ...
                // std::cout << "SPIN SOME: Time " << std::setw(9) << " : " << diff.count() << " s\n";

                rclcpp::Duration ros_diff = ros_end - ros_start;
                // std::cout << "SPIN SOME: ROS Time " << std::setw(9) << " : " << ros_diff.nanoseconds() << " ns\n";
...
protected:
    ...
    void send_new_goal()
    {
        ...
        //     [this](std::shared_future<typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr> future)
        // {
        ...
        send_goal_options.result_callback = std::bind(&BtActionNode::result_callback, this, std::placeholders::_1);
    ...
    bool is_future_goal_handle_complete(std::chrono::milliseconds &elapsed)
    {
        ...
        if (result == rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            // TODO: be careful, result will be always successful
            // goal_result_available_ = true;
